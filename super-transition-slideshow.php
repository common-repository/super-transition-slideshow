...
Description: Dont just display images, showcase them in style using this Super transition slideshow plugin. Randomly chosen Transitional effects in IE browsers.  
...
Text Domain: super-transition-slideshow
...
function sts_show() 
{
	...
	$sts_pluginurl = plugins_url() . "/super-transition-slideshow/";
...
function sts_install() 
{
	...
	add_option('sts_dir_1', "wp-content/plugins/super-transition-slideshow/images/");
	add_option('sts_dir_2', "wp-content/plugins/super-transition-slideshow/images/");
	add_option('sts_dir_3', "wp-content/plugins/super-transition-slideshow/images/");
	add_option('sts_dir_4', "wp-content/plugins/super-transition-slideshow/images/");
}
	...
	<div class="wrap">
	  <div class="form-wrap">
		...
		if (isset($_POST['sts_form_submit']) && $_POST['sts_form_submit'] == 'yes')
		{
			...
			$sts_title 		= stripslashes(sanitize_text_field($_POST['sts_title']));
			$sts_dir 		= stripslashes(sanitize_text_field($_POST['sts_dir']));
			$sts_title_yes 	= stripslashes(sanitize_text_field($_POST['sts_title_yes']));
			$sts_pause 		= stripslashes(intval($_POST['sts_pause']));
			$sts_transduration 	= stripslashes(intval($_POST['sts_transduration']));
			$sts_dir_1 		= stripslashes(sanitize_text_field($_POST['sts_dir_1']));
			$sts_dir_2 		= stripslashes(sanitize_text_field($_POST['sts_dir_2']));
			$sts_dir_3 		= stripslashes(sanitize_text_field($_POST['sts_dir_3']));
			$sts_dir_4 		= stripslashes(sanitize_text_field($_POST['sts_dir_4']));
			
			if(!is_numeric($sts_pause) || $sts_pause == 0) { $sts_pause = 2000; }
			if(!is_numeric($sts_transduration) || $sts_transduration == 0) { $sts_transduration = 1000; }
		...
		<form name="sts_form" method="post" action="">
			...
			<a class="button" target="_blank" href="http://www.gopiplus.com/work/2010/07/18/super-transition-slideshow/"><?php _e('Help', 'super-transition-slideshow'); ?></a>&nbsp;
			<a class="button" target="_blank" href="http://www.gopiplus.com/work/2010/07/18/super-transition-slideshow/"><?php _e('Short Code', 'super-transition-slideshow'); ?></a>
...
function sts_add_javascript_files() 
{
	if (!is_admin())
	{
		wp_enqueue_script( 'super-transition-slideshow', plugins_url().'/super-transition-slideshow/super-transition-slideshow.js');
	}
}
